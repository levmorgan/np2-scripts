...
np2_long$in_benchmark <- np2_long$genes %in% benchmark[,2]
...
plot_data = c()
for (i in 1:100) {
	support_lists <- Map(function (genes) {
				     edges <- genes[1:i,]
				     edges <- edges[edges$in_benchmark,]
				     return(edges$has_support)
				     
		 }, 
				     np2_split)
	support_sums <- as.vector(Map(sum, support_lists), mode="numeric") 
	support_lengths <- sapply(support_lists, length)
	support_percent <- sum(support_sums)/sum(support_lengths)
	
	plot_data[i] <- support_percent
}
